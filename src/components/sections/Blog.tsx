...
const Blog: React.FC<BlogProps> = ({ posts, onLike, onLove, onReadMore }) => {
  ...
  // Filtramos apenas posts que têm imagem válida para não mostrar o quadro cinza vazio
  const validPosts = posts.filter(
    (post) => !errorImages[post.id] && post.imageUrl,
  );
    ...
    <Section id="blog" title="Blog do Pedro" isDark>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        {validPosts.length > 0 ? (
          ...
          <div className="col-span-full py-24 text-center text-white/10 border-2 border-dashed border-white/5 rounded-3xl">
